...
public class JSWaiter {
    ...
    public static void waitForJSLoad() {
        ...
        // Get JS is Ready
        boolean jsReady = jsExec.executeScript("return document.readyState").toString().trim()
        ...
        if (!jsReady) {
            ...
            while ((!jsReady) && (tryCounter < 5)) {
                ...
                sleep(20);
                tryCounter++;
                jsReady = jsExec.executeScript("return document.readyState").toString().trim()
                        .equalsIgnoreCase("complete");
            }
            if (debug) {
        ...
        try {
            ...
            if (iframes > 0) {
                for (int i = 0; i < iframes; i++) {
                    Boolean jsReady = jsExec.executeScript(
